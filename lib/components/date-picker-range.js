...
exports.default = {
    props: {
        ...
        options: {
            ...
            default: function _default() {
                return {
                    cellContent: function cellContent(d) {
                        return d.getDate();
                    }
                };
            }
    ...
    methods: {
        ...
        initDays: function initDays() {
            ...
            for (var i = 0, l = rows.length; i < l; i++) {
                ...
                var _loop = function _loop(j) {
                    ...
                    cell.index = index;
                    cell.text = function () {
                        return _this.options.cellContent(d);
                    };
                    cell.type = time < minTime ? 'prev-month' : time > maxTime ? 'next-month' : 'normal';
                    cell.isToday = time === (0, _utils.getClearHoursTime)(Date.now());
                    cell.isSelected = (0, _utils.isDate)(_this.selectedDate) ? time === (0, _utils.getClearHoursTime)(new Date(_this.selectedDate).getTime()) : false;
                    cell.date = d;

                    // disable date
                    if (_this.options && typeof _this.options.disabledDate === 'function') {
                        cell.disabled = _this.options.disabledDate(cell.date);
                    }

                    cell.start = _this.isStartDate(d);
                    cell.end = _this.isEndDate(d);
                    cell.inRange = _this.isInRange(d);
        ...
        selectdCurDate: function selectdCurDate(e) {
            ...
            if (index) {
                var _cell = this.getCellInfoByIndex(index);
                if (_cell.type === 'normal' && !_cell.disabled) {
                    var minTime = (0, _utils.getClearHoursTime)(this.minDate);
                    var maxTime = (0, _utils.getClearHoursTime)(_cell.date.getTime());

                    this.curDate = _cell.date;
                    this.selectedDate = (0, _utils.formatDate)(_cell.date, this.format);
                    this.$emit('range-change', _cell.date, maxTime < minTime);
                }
            }
        },
        markRange: function markRange(maxDate) {
            ...
            for (var i = 0, l = rows.length; i < l; i++) {
                ...
                for (var j = 0; j < 7; j++) {
                    ...
                    _cell2.start = time === minTime;
                    _cell2.end = time === maxTime && maxTime >= minTime;
                    _cell2.inRange = time >= minTime && time <= maxTime;
                }
                rows[i] = row;
            }
            this.rows = [].concat(rows);
        ...
        handleMouseMove: function handleMouseMove(e) {
            if (this.selecting) {
                var index = e.target.dataset ? e.target.dataset.index : e.target.getAttribute('data-index');
                if (index) {
                    var _cell3 = this.getCellInfoByIndex(index);
                    this.$emit('end-date-change', _cell3.date);
                }
            }
        }
    },
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_eba8acd0_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_date_table_vue__ = __webpack_require__(25);
var normalizeComponent = __webpack_require__(0)
/* script */
...
var Component = normalizeComponent(
  ...
  __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_eba8acd0_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_date_table_vue__["a" /* default */],
  __vue_template_functional__,
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
